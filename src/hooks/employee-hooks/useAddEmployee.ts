...
export const DEPARTMENTS = [
    'Director', 'Engineering', 'Sales', 'Marketing', 'Finance', 'Operations', 'HR', 'IT', 'Accountant', 'Area Manager', 'Driver', 'Helper', 'Office Boy', 'Wealth Advisor', 'BDM'
];

export const POSITIONS = [
    'Intern', 'Junior Developer', 'Software Developer', 'Tester', 'Android Developer', 'iOS Developer', 'App Developer', 'Senior Developer', 'Manager', 'Director', 'VP', 'General Manager', 'Specialist', 'HR Executive', 'System Administrator'
];
...
export const useAddEmployee = () => {
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            try {
                ...
                if (isMounted) {
                    setFormData(prev => ({ ...prev, employeeCode: codeRes?.data || codeRes || '' }));

                    const managers = managersRes?.data || managersRes || [];
                    setManagerOptions(managers.map((m: any) => ({
                        label: m.name,
                        value: m._id
                    })));
                }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            data.append('alternateMobileNumber', formData.alternateMobileNumber || '');
            ...
            data.append('emergencyContactRelationship', formData.emergencyContactRelationship);
            ...
            // Handle nested Address object mapping packages definitions
            data.append('address[current][address]', formData.address.current.address);
            data.append('address[current][pinCode]', formData.address.current.pinCode);
            data.append('address[current][city]', formData.address.current.city);
            data.append('address[current][district]', formData.address.current.district || formData.address.current.city);
            data.append('address[current][state]', formData.address.current.state);

            data.append('address[permanent][address]', formData.address.permanent.address);
            data.append('address[permanent][pinCode]', formData.address.permanent.pinCode);
            data.append('address[permanent][city]', formData.address.permanent.city);
            data.append('address[permanent][district]', formData.address.permanent.district || formData.address.permanent.city);
            data.append('address[permanent][state]', formData.address.permanent.state);
            ...
            // console.log("Submitting validated FormData payload packages directly over to NestJS client gateways...");
            await employeeService.createEmployee(data);
            router.back();
        } catch (error) {
            console.error("Submission operational mapping integration runtime execution error point hit:", error);
            alert("Failed to serialize or deliver data over to the collection context pipelines.");
        } finally {
